...
```{r datatemp1, echo = TRUE,eval=TRUE}
head(datatemp)
##dd
```
...
## **Températures par grandes Rg**

Les temperatures des quatres grandes Rg sont : 

 
```{r tempreg, echo = F}
moyTR<-round(tapply(datatemp$Moye,datatemp$Région,mean),2)
maxAR<-tapply(datatemp$Ampl,datatemp$Région,max)
dd<-data.frame(Rg=levels(datatemp$Région),Moyenne=moyTR,AmplitudeM=maxAR)
```
